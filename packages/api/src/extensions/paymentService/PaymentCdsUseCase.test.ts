...
describe('Payment Cds Usecase', () => {
  ...
  describe('setPaymentInformation', () => {
    it('payment method is cash on delivery: success', async () => {
      ...
      const paymentResult = await paymentCdsUseCase.setPaymentInformation(
        root,
        inputPaymentCashOnDelivery,
        context,
        info,
    ...
    it('payment method is saved credit card: throw error when encrypted card is empty', async () => {
      ...
      await expect(() =>
        paymentCdsUseCase.setPaymentInformation(root, inputPaymentSavedCreditCardWithoutEncryptedCard, context, info),
      ).rejects.toThrow();
    });
    ...
    it('payment method is credit card: throw error when user is guest', async () => {
      ...
      await expect(() =>
        paymentCdsUseCase.setPaymentInformation(root, inputPaymentCreditCardWithoutCard, context, info),
      ).rejects.toThrow();
    });
  });
});
